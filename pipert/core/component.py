...
import zerorpc
# import weakref
from .errors import RegisteredException, ZrpcNotBound
...
class BaseComponent:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._routines = []
        self._zrpc = None
        self._input_keys = defaultdict(dict)
        self._output_keys = defaultdict(dict)
        self._conf = defaultdict(dict)

    def _bind_endpoint(self, endpoint):
        self.endpoint = endpoint
        self._zrpc = zerorpc.Server(self)
        self._zrpc.bind(self.endpoint)
    ...
    def run(self):
        ...
        """
        if self._zrpc is None:
            raise ZrpcNotBound(f"component don't have a zrpc endpoint")
        self._start()
    ...
    def register_routine(self, routine: Union[Routine, Process, Thread]):
        ...
        if isinstance(routine, Routine):
            if routine.stop_event is None:
                routine.stop_event = self._stop_event
            else:
