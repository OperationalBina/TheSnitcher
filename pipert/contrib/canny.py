...
from pipert.core.mini_logics import FramesFromRedis, Frames2Redis
import zerorpc
import argparse
...
class Canny(BaseComponent):
    ...
    def __init__(self, out_key, in_key, redis_url, field, maxlen):
        ...
        t_get = FramesFromRedis(in_key, redis_url, self.in_queue, self.field, name="get_frames").as_thread()
        self.register_routine(t_get)
        t_det = CannyLogic(self.in_queue, self.out_queue, True, name="canny").as_thread()
        self.register_routine(t_det)
        t_send = Frames2Redis(out_key, redis_url, self.out_queue, maxlen, name="send_frames").as_thread()
...
if __name__ == '__main__':
    ...
    zpc = Canny(args.output, args.input, url, args.field, args.maxlen)
